...
/client/New(TopicData)
	...
	if (isnum(cached_player_age) && cached_player_age == -1) //first connection
		if (nnpa >= 0)
			message_admins("New user: [key_name_admin(src)] is connecting here for the first time. (BYOND account created on [account_join_date])")
			send2irc("New-user", "[key_name(src)] is connecting for the first time! (BYOND account created on [account_join_date])")
			if (CONFIG_GET(flag/irc_first_connection_alert))
				send2irc_adminless_only("New-user", "[key_name(src)] is connecting for the first time! (BYOND account created on [account_join_date])")
...
/client/proc/set_client_age_from_db(connectiontopic)
	...
	if(!query_client_in_db.NextRow())
		if (CONFIG_GET(flag/panic_bunker) && !holder && !GLOB.deadmins[ckey] && !(ckey in GLOB.bunker_passthrough))
			var/datum/DBQuery/query_client_in_db_whitelist = SSdbcore.NewQuery("SELECT 1 FROM [format_table_name("bunker")] WHERE ckey = '[ckey]'")
			...
			if(!query_client_in_db_whitelist.NextRow()) //they are not on the whitelist OR have played before! on the database!
				...
				to_chat(src, "<span class='notice'>You must first join the Discord to verify your account before joining this server.  To do so, read the rules in the 'server essentials' > #whitelist-application section of the discord which can be joined here:<a href='https://discord.gg/MqmgY8a'>https://discord.gg/MqmgY8a</a></span>") //CIT CHANGE - makes the panic bunker disconnect message point to the discord
